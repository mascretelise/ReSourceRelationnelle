...
const [serverError, setServerError] = useState("")
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setServerError("");
    try {
      const response = await fetch(`http://localhost:3000/api/category/editCategory?id=${catId}`, {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    <div>
      <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
        <Controller
          ...
          render={({ field }) => (
            <div className="">
              <label className="block text-sm/6 font-medium text-gray-900">Modifier la catégorie</label>
              <div className="mt-2">
        ...
        <div className="flex flex-col justify-self-center">
          ...
          type="submit">Envoyer la modification</button>
        </div>
    </form>
    </div>
  );
